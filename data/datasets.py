...
from .process import get_processing_model, processing_LGrad, processing_LNP, processing, processing_DCT, Get_Transforms
...
class AIGCImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        elif self.opt.detection_model in ['GramNet', 'CNNSpot', 'NPR', 'SAFE', 'SimE', 'FatFormer']:
            img = processing(img, self.opt, 'imagenet')
        elif self.opt.detection_model == 'FreDect':
            img = processing_DCT(img, self.opt)
        elif self.opt.detection_model == 'UnivFD':
